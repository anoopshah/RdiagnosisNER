...
findLaterality <- function(D, i){
	...
	if (max(i) < nrow(D) - 2){
		# ... left and right
		if (D$lemma[max(i) + 1] %in% lat_phrases &
			D$lemma[max(i) + 2] == 'and' &
			D$lemma[max(i) + 3] %in% lat_phrases){
			i_lat <- c(max(i) + 1, max(i) + 3)
		}
	}
	if (min(i) > 3){
		# left and right ...
		if (D$lemma[min(i) - 3] %in% lat_phrases &
			D$lemma[min(i) - 2] == 'and' &
			D$lemma[min(i) - 1] %in% lat_phrases){
			i_lat <- c(min(i) - 1, min(i) - 3)
		}
	}
	if (min(i) > 2){
		# left/right sided ...
		if (D$lemma[min(i) - 2] %in% lat_phrases &
			D$lemma[min(i) - 1] %in% c('side', 'sided')){
			i_lat <- min(i) - 2
		}
	}
