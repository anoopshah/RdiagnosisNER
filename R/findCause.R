...
findCause <- function(D, i){
	...
	if (max(i) < nrow(D) - 1){
		# ... by/after/follow X
		if (tolower(D$lemma[max(i) + 1]) %in% c('by', 'after', 'follow')){
			i_cause <- max(i) + 2
		}
	}
	if (max(i) < nrow(D) - 2){
		# ... due to/cause by/because of X
		if (tolower(paste(D$lemma[max(i) + 1], D$lemma[max(i) + 2])) %in%
	...
	if (max(i) < nrow(D) - 3){
		# ... as result of X
		if (tolower(paste(D$lemma[max(i) + 1], D$lemma[max(i) + 2],
			D$lemma[max(i) + 3])) %in%
	...
	if (min(i) > 3){
		# X resulting in ...
		if (tolower(paste(D$lemma[min(i) - 2], D$lemma[min(i) - 1])) %in%
	...
	if (min(i) > 2){
		# X causing ..., X related
		if (tolower(D$lemma[min(i) - 1]) %in% c('cause', 'relate', 'related')){
			i_cause <- min(i) - 2
		}
		# post X ...
		if (tolower(D$lemma[min(i) - 2]) == 'post'){
			i_cause <- min(i) - 1
		}
	}
